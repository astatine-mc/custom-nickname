...
- 코드 실행·테스트 전에 현재 변경을 검토하여 체크포인트 커밋을 만들고 GitHub에 push한다. 변경이 없으면 빈 체크포인트 커밋을 허용한다.
- 요청받은 코드가 완성되고 검증을 마치면 관련 소스·테스트·문서를 자동으로 커밋하고 기존 GitHub 원격 브랜치에 push한다. 매번 별도 확인을 요청하지 않는다.
- 비밀번호·토큰·실행 로그·target 산출물·무관한 사용자 변경은 커밋하지 않는다. 인증 실패나 원격 충돌은 강제 push로 덮지 않고 원인을 보고한다.
- 완료 보고에는 체크포인트와 최종 커밋, 검증 결과, 남은 제약을 명시한다.
