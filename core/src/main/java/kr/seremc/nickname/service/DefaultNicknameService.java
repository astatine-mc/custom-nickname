...
public final class DefaultNicknameService implements NicknameService,AutoCloseable {
 ...
 private <T>CompletableFuture<T> async(Task<T> task){try{return CompletableFuture.supplyAsync(()->{try{return task.run();}catch(Exception e){throw new CompletionException(e);}},executor);}catch(RejectedExecutionException e){return CompletableFuture.failedFuture(new IllegalStateException("요청이 많습니다. 잠시 후 다시 시도하세요."));}}
